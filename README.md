...
- 📚 **Library** — a shelf of **15 decodable readers** (every page gated through the real validator). Page-turn reader, tap-any-word audio, "Read to me" with word highlighting; finishing a book pays a reward once. Reading a page logs `sentence_read` — the strongest reading signal the game collects. **Books are tier-gated**: the 5 blend-tier readers stay off the shelf until their sounds are unlocked, so the Library grows as the child's reading advances (a locked-count hint shows how many more await).
