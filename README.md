...
- ~290-word decodable corpus enforced by a build-time validator (the iron rule)
...
- 📚 **Library** — a shelf of **17 decodable readers** (every page gated through the real validator). Page-turn reader, tap-any-word audio, "Read to me" with word highlighting; finishing a book pays a reward once. Reading a page logs `sentence_read` — the strongest reading signal the game collects. **Books are tier-gated**: the 7 higher-tier readers (5 blend, 1 magic-e, 1 vowel-team) stay off the shelf until their sounds are unlocked, so the Library grows as the child's reading advances (a locked-count hint shows how many more await).
